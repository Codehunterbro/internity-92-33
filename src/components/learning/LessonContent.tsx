      ...
      return (
        <div className="space-y-4">
          {videoResources.map(video => (
            <div key={video.id} className="bg-white rounded-lg shadow-md overflow-hidden mb-4 lg:mb-6">
    ...
    <div className="p-4 lg:p-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-4 lg:mb-6 w-full grid grid-cols-3 h-auto">
          <TabsTrigger value="content" className="flex items-center gap-1 lg:gap-2 text-xs lg:text-sm py-2">
            <BookOpen className="h-3 w-3 lg:h-4 lg:w-4" />
            <span className="hidden sm:inline">Content</span>
          </TabsTrigger>
          <TabsTrigger value="resources" className="flex items-center gap-1 lg:gap-2 text-xs lg:text-sm py-2">
            <FileText className="h-3 w-3 lg:h-4 lg:w-4" />
            <span className="hidden sm:inline">Resources</span>
          </TabsTrigger>
          <TabsTrigger value="quiz" className="flex items-center gap-1 lg:gap-2 text-xs lg:text-sm py-2">
        ...
        <TabsContent value="resources" className="p-0 mt-0">
          {resources.length > 0 ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3 lg:gap-4">
                {resources.map(resource => (
                  <div key={resource.id} className="border rounded-md p-3 lg:p-4 flex items-center">
                    {resource.type.toLowerCase().includes('video') ? (
                      <Video className="h-8 w-8 lg:h-10 lg:w-10 text-brand-purple mr-3 lg:mr-4 flex-shrink-0" />
                    ) : (
                      <FileText className="h-8 w-8 lg:h-10 lg:w-10 text-brand-purple mr-3 lg:mr-4 flex-shrink-0" />
                    )}
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-sm lg:text-lg truncate">{resource.name}</h3>
                      <p className="text-xs lg:text-sm text-gray-500">{resource.type} · {resource.size}</p>
                    </div>
                    <a 
                      href={resource.url} 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="bg-brand-purple hover:bg-brand-purple/90 text-white py-1.5 px-2 lg:py-1 lg:px-3 rounded text-xs lg:text-sm whitespace-nowrap ml-2"
                    >
            ...
            <div className="text-center py-8 lg:py-10">
              <FileText className="h-10 w-10 lg:h-12 lg:w-12 mx-auto text-gray-400 mb-3 lg:mb-4" />
        ...
        <TabsContent value="quiz" className="p-0 mt-0">
          {isLoadingQuiz ? (
            ...
            <div className="text-center py-8 lg:py-10">
              <HelpCircle className="h-10 w-10 lg:h-12 lg:w-12 mx-auto text-gray-400 mb-3 lg:mb-4" />
