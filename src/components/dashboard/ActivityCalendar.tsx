...
const ActivityCalendar: React.FC<ActivityCalendarProps> = ({
  ...
  return <div className="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div className="min-w-max">
          ...
          <div className="flex mb-3">
            ...
            <div className="flex flex-1 justify-between">
              {months.map((month, idx) => <div key={idx} className="px-2 text-sm font-medium">{month}</div>)}
            </div>
          </div>
          ...
          <div className="flex">
            ...
            <div className="flex flex-1 gap-1">
              {weeksData.map((week, weekIdx) => <div key={weekIdx} className="flex flex-col gap-1">
                  {week.map((date, dayIdx) => {
                if (!date) {
                  return <div key={dayIdx} className="w-6 h-6"></div>;
                }
                return <div key={dayIdx} className={`w-6 h-6 rounded-sm ${getDayColor(date)} hover:ring-2 hover:ring-blue-400 cursor-pointer transition-all`} onMouseEnter={e => handleMouseEnter(e, date)} onMouseLeave={handleMouseLeave} title={`${date.toLocaleDateString()} - ${attendanceData[date.toISOString().split('T')[0]] || 'No data'}`}></div>;
              })}
                </div>)}
            </div>
          </div>
      ...
      {/* Tooltip */}
      {tooltip.show && <div className="fixed z-50 bg-gray-800 text-white px-2 py-1 rounded text-xs pointer-events-none transform -translate-x-1/2 -translate-y-full" style={{
      left: tooltip.x,
      top: tooltip.y
    }}>
          {tooltip.content}
